...
>Указыват все версии файла, позволяет увидеть сколько было изменений и комментарии к сохранениям.
...
> Для работы необходиомо указать интересующий коммит, и выбрать тот, в котором желаем остаться. При помощи команды git checkout master
<br/><br/>
 ...
 * # `git clone` - команда позволяет создать клон внешнего репозитория на ПК
 ...
 * # `git push` - команда позволяет отправить нашу версию репозитория (требуется авторизация на внешнем репозитории)
 <a href="https://ibb.co/BsZ7b4Y"><img src="https://i.ibb.co/NYjk51G/git-push.jpg" alt="git-push" border="0"></a>
  <br/><br/>

 * # `git commit --amend` -команда для корректировки commit(в случае если допустили опечатку или ошибку)
 <br/><br/>
